...
class TRAIN_DDPG(TRAIN):
    ...
    def PlotTrainData(self): 
        ...
        # Plot total value
        Reader = er()
        Reader.LoadFile("./Data/Temp_data/DDPG_reward.xlsx", header = None)
        plt.figure()
        plt.ylabel('Total value')
        plt.xlabel('Trajectory')
        plt.plot(np.linspace(1,1999,1999), Reader.DataFrame.iloc[:,0],lw='1')
        plt.show()

        # Plot steps
        Reader.LoadFile("./Data/Temp_data/DDPG_step.xlsx", header = None)
        plt.figure()
        plt.ylabel('Steps')
        plt.xlabel('Trajectory')
        plt.plot(np.linspace(1,1999,1999), Reader.DataFrame.iloc[:,0],lw='1')
...
class TRAIN_SPRING(TRAIN_DDPG):
    ...
    def PlotTestData(self, path = './Data/DDPG/Spring_data_in_paper/DDPG_x.xlsx'):
        
        # Plot the displacement
        Reader = er()
        Reader.LoadFile(path, Header = None)    # It can be replaced by other testing data
        row = Reader.DataFrame.shape[0]
        plt.figure()
        plt.ylabel('x(m)')
        plt.xlabel('Steps')
        plt.plot(np.linspace(1, row, row), Reader.DataFrame.iloc[:,0],lw='1')
...
class TRAIN_MANIPULATOR(TRAIN_DDPG):
    ...
    def PlotTestData(self, path = './Data/DDPG/Manipulator_data_in_paper/DDPG_state.xlsx'):
        ...
        plt.xlabel('Steps')
        plt.plot(np.linspace(1, row, row), Reader.DataFrame.iloc[:,0],lw='1')
        plt.plot(np.linspace(1, row, row), Reader.DataFrame.iloc[:,1],lw='1')
        plt.axhline(0.0, color='red', linestyle='--')
        plt.legend([r"$eta_1$", r"$eta_2$"])
        plt.show()
...
class TRAIN_AL_SPRING(TRAIN_AL):
    ...
    def PlotTestData(self, path = './Data/AL/Spring_data_in_paper/Lyp_x.xlsx'):
        ...
        plt.xlabel('Steps')
        for i in range(10):
            plt.plot(np.linspace(1, row, row), Reader.DataFrame.iloc[:,i],lw='1')
        plt.axhline(0.0, color='red', linestyle='--')
        plt.show()
...
class TRAIN_AL_MANIPULATOR(TRAIN_AL):
    ...
    def PlotTestData(self, path = './Data/AL/Manipulator_data_in_paper/Lyp_state.xlsx'):
